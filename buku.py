        ...
        while True:
            ...
            if pilihan == '1':
                try:
                    ...
                    continue
                books = load_csv(BUKU_FILE)
                result = knapsack(budget, books)
                headers = ['Judul', 'Penulis', 'Tahun', 'Harga']
                table = [[b['judul'], b['penulis'], b['tahun'], b['harga']] for b in result]
...
def admin_dashboard():
    while True:
        ...
        if pilihan == '1':
            ...
            for b in books:
                if b['judul'].strip().lower() == new_judul.lower():
                    try:
            ...
            found = False
            for b in books:
                if b['judul'].strip().lower() == judul.lower():
                    b['tahun'] = input("Tahun: ")
            ...
            books = [b for b in books if b['judul'].lower() != judul.lower()]
            save_csv(BUKU_FILE, books, ['judul', 'tahun', 'penulis', 'stok', 'harga'])
            print("Buku dihapus.")
